...
public class TransactionServiceimpl implements TransactionService {
	...
	private static final String[] ALLOWED_EXTENSION = new String[] { ".hwp", ".doc", ".docx", ".ppt", ".pptx", ".xls",
			".xlsx", ".txt", ".csv", ".jpg", ".jpeg", ".gif", ".png", ".bmp", ".pdf" };
	...
	public String insertTransaction(TransactionDTO dto) throws Exception {
		...
		// insert 후 transaction_idx 추출 (useGeneratedKeys="true"
		// keyProperty="transaction_idx")
	...
	@Transactional(rollbackFor = Exception.class)
	public boolean insertTransactionDetails(String transaction_idx, JSONArray itemJsonArray, JSONArray memoJsonArray) {

		if (!insertItemList(transaction_idx, itemJsonArray))
			return false;
		if (!insertMemoList(transaction_idx, memoJsonArray))
			return false;
	...
	/**
	 * 거래 내역에 등록된 파일을 저장하는 메서드 매개변수로 넘어온 파일을 견적서(quote), 지시서(order), 이미지(image),
	 * 기타(other)파일로 분류 파일 이름 앞에 날짜를 붙여서 지정된 경로에 저장한 후 DB에 insert
	 */
	...
	/**
	 * 중복된 파일의 이름을 변경하는 메서드 해당 경로(path)에 동일한 파일명(fileName)의 파일이 존재하는지 확인하여
	 * 존재한다면 파일명 뒤에 "-복사본"을 붙인후 변경된 이름(fileName)을 반환 변경된 이름을 다시 체크하여 동일한 파일명이 존재하지
	 * 않을 때 까지 반복한다.
	 */
	...
	/**
	 * 파일명 앞에 날짜(yyyy-Mm-dd)를 붙이는 메서드 파일명(fileName)앞에 날짜가 없으면 매개변수로 입력된 날짜(date)를
	 * 붙인다 파일명(fileName)앞에 이미 날짜가 붙어 있으면 날짜 제거 후 매개변수로 입력된 날짜(date)를 붙인다
	 */
	...
	public boolean updateItemList(String transaction_idx, JSONArray jsonArray) {
		...
		try {
			...
			// item 목록 새로 입력
			if (insertItemList(transaction_idx, jsonArray))
				return true;
	...
	public boolean updateMemoList(String transaction_idx, JSONArray jsonArray) {
		...
		try {
			...
			// memo 목록 새로 입력
			if (insertMemoList(transaction_idx, jsonArray))
				return true;
	...
	@Transactional(rollbackFor = Exception.class)
	public boolean updateTransactionDetails(String transaction_idx, JSONArray itemJsonArray, JSONArray memoJsonArray) {

		if (!updateItemList(transaction_idx, itemJsonArray))
			return false;
		if (!updateMemoList(transaction_idx, memoJsonArray))
			return false;
	...
	public void updateTransactionFiles(String date, String company_idx, String transaction_idx,
		...
		for (int i = 0; i < storedFileList.size(); i++) {
			...
			for (int j = 0; j < existingFilejsonArray.size(); j++) {
				...
				if (storedFileList.get(i).getFile_idx().equals(obj.get("file_idx"))) {
					...
					if (!newName.equals(storedFileList.get(i).getFile_name())) {
						// 이름이 변경된 경우에만 변경
						oldFile = FileUtils.getFile(file_root + File.separator + companyFolderName + File.separator
			...
			if (!isFileExist) {
				...
				oldFile = FileUtils.getFile(file_root + File.separator + companyFolderName + File.separator
						...
						+ storedFileList.get(i).getFile_type().getFolder_name() + File.separator
						+ storedFileList.get(i).getFile_name());
				newFile = FileUtils.getFile(file_root + File.separator + "휴지통" + File.separator + "("
						+ storedFileList.get(i).getFile_idx() + ")" + storedFileList.get(i).getFile_name());
				FileUtils.moveFile(oldFile, newFile); // 파일 이동 + 경로 자동 생성
		...
		// [새로운 파일 추가]
		insesrtTransactionFiles(date, company_idx, transaction_idx, multipartRequest);
